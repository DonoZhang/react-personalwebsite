...
export default (WrappedComponent)=>{
    class CollapsibleWrapper extends Component{
			static propTypes={
				...
				collapse: PropTypes.bool,//set the initial/accordion collapse/show status
				accordion: PropTypes.bool,//when enabled, collapsibility is controled by this.props.collapse rather than this.state.collapse
				onClickBtn: PropTypes.func//communication with outside components
			}
			...
			_getHeight(){
				...
				if(this.props.enabled){
					...
					if(this.props.accordion) height=this.props.collapse?'0':'auto'
					//normal model, height depends on this.state.collapse
					else height=this.state.height
				}
			...
			render(){
				return (
					<div>
						<button onMouseUpCapture={this.onClickBtn} className='collapse-button' style={
							{...this.props.btnDisplay, display: this.props.enabled?'block':'none'/*if disabled, hide the button */}
						}>{this.props.buttonName}</button>
						<div className = 'collapse' ref={(div)=>this.div = div} style={{overflow: 'hidden', height: this._getHeight()}}>
